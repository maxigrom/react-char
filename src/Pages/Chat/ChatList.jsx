...
class ChatList extends React.Component<Props, State> {
  ...
  render() {
    ...
    const {
      disabled, allChats, myChats, activeChat, classes,
    } = this.props;
      ...
      <>
        ...
        <div className={classes.toolbar}>
          <div className={classes.searchChats}>
            <Input fullWidth placeholder="Search chats..." />
          </div>
        </div>
        <Divider />
        <List className={classes.chatList}>
          {displayChatList.map(chat => (
            <ChatListItem key={chat._id} chat={chat} disabled={disabled} isActive={!!activeChat && chat._id === activeChat._id} />
          ))}
        </List>
        <Divider />
